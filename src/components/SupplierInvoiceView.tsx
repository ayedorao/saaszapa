...
export default function SupplierInvoiceView({ invoiceId, onClose }: SupplierInvoiceViewProps) {
  ...
  const statusText = invoice.status === 'draft' ? 'PENDIENTE DE PAGO' : invoice.status === 'confirmed' ? 'PAGADO' : 'CANCELADO';
  const statusColor = invoice.status === 'draft' ? 'text-red-700 bg-red-100' : invoice.status === 'confirmed' ? 'text-green-700 bg-green-100' : 'text-slate-700 bg-slate-100';
